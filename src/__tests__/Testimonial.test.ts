...
describe('Testimonial', () => {
  ...
  describe('Create Testimonial', () => {
    ...
    test('User should be able to post a testimonial to a expert', async () => {
      const response = await request(app).post('/testimonials')
        .send({
          userId: user.id,
          expertId: expert.id,
          description: 'Ótimo rapaz, simpático e muito prestativo!',
          stars: 5.0,
        });
    ...
    test('Expert score must be updated after the post of a new testimonial rating', async () => {
    ...
    test('User should not be able to post a testimonial to a expert that already testified', async () => {
      const response = await request(app).post('/testimonials')
        .send({
          userId: user.id,
          expertId: expert.id,
